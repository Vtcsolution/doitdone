    ...
    const fetchSavedReports = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          toast.error("Failed to fetch astrology reports.");
          setErrorMessage(response.data.message || "Failed to fetch astrology reports.");
        }
      } catch (error) {
        console.error("Failed to fetch astrology reports:", error);
          ...
          <table className="w-full table-auto border-collapse min-w-[640px]">
            ...
            <tbody>
              {savedReports.map((report, index) => (
                ...
                >
                  <td className="px-2 sm:px-4 py-2 sm:py-3 text-gray-700 dark:text-gray-300 text-xs sm:text-sm truncate">{report.chart?.moon?.sign || "Unknown"}</td>
